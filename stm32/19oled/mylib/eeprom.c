...
void eeprom_page_write(u8 daddr, u8 waddr, u8 *buf, u8 size)//按页来写
{
	...
	IIC_Start();//产生开始信号
	IIC_Send_Byte(daddr);//发送从设备的设备地址
	while(IIC_Wait_Ack());//等待ACK
	IIC_Send_Byte(waddr);//发送内部存储空间的地址
	while(IIC_Wait_Ack());//等待ACK
	for(i = 0; i < size; i++)
...
{
	u8 data = 0;//通过data变量接收读取到的数据
	
	IIC_Start();//产生开始信号
	IIC_Send_Byte(daddr | 1);//发送设备地址 + 读的标志
	while(IIC_Wait_Ack());//等待ACK
	data = IIC_Recv_Byte(0);//读取数据,并且不产生ACK
	IIC_Stop();//产生结束信号
...
void eeprom_sequ_read(u8 daddr, u8 waddr, u8 *buf, u8 size)//顺序地址读
{
	...
	{
		if(i == size - 1)
			buf[i] = IIC_Recv_Byte(0);//读取最后一个字节数据,不产生ACK
		else
			buf[i] = IIC_Recv_Byte(1);//读取数据,产生ACK
	}
	IIC_Stop();//产生结束信号
}
